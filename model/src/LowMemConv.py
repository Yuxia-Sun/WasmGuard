...
def drop_zeros_hook(module, grad_input, grad_out):
    ...
    So we use this as a approximation to sparse BP to avoid redundant and useless work
    """
    grads = []
    with torch.no_grad():
        for g in grad_input:
...
class LowMemConvBase(nn.Module):
    ...
    def __init__(self, chunk_size=65536, overlap=512, min_chunk_size=1024):
        ...
        but use more memory. Total memory use will be a function of chunk_size,
        and not of the length of the input sequence L
        ...
        # Used for pooling over time in a memory efficent way
    ...
    def processRange(self, x, embeded=False, **kwargs):
        """
        This method does the work to convert an LongTensor input x of shape (B, L) ,
        where B is the batch size and L is the length of the input. The output of
        this functoin should be a tensor of (B, C, L), where C is the number of channels,
        and L is again the input length (though its OK if it got a little shorter due to
        convs without padding or something).
        """
        pass
    ...
    def determinRF(self, embeded):
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                is_valid = True
                try:
                    self.processRange(tmp[:, 0:test_size], embeded)
                except:
                    is_valid = False
                ...
                if max_rf == min_rf:
                    ...
                    # print("processRange embeded: ", embeded)
                    out_shape = self.processRange(tmp, embeded).shape
    ...
    def seq2fix(self, x, embeded=False, pr_args={}):
        """
        ...
        receptive_window, stride, out_channels = self.determinRF(embeded)

        # print("receptive_window, stride, out_channels: ", receptive_window, stride, out_channels)
        ...
        with torch.no_grad():
            while start < end and (end - start) >= max(self.min_chunk_size, receptive_window):
                ...
                activs = self.processRange(x_sub, embeded, **pr_args)
        ...
        x_selected = self.processRange(x_selected, embeded, **pr_args)
